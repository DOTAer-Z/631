<template>
  <div class="log-analysis-page">
    <el-card shadow="never" class="result-card">
      <template #header><span>页面入口</span></template>
    ...
    <el-card v-show="activeSection === 'analysis'" shadow="never" class="result-card analysis-pane">
      <template #header><span>快速日志分析</span></template>
      <el-form label-position="top" class="quick-analysis-form">
        <el-form-item label="文件名（可选）">
          <el-input
            v-model="quickAnalysis.filename"
            clearable
            placeholder="manual_input.log"
          />
        </el-form-item>
        <el-form-item label="日志内容">
          <el-input
            v-model="quickAnalysis.logText"
            type="textarea"
            :rows="8"
            placeholder="在此粘贴需要快速分析的日志内容..."
          />
        </el-form-item>
        <div class="analysis-actions">
          <el-button type="primary" :loading="analyzing" @click="handleQuickAnalyze">分析日志</el-button>
          <el-button :disabled="analyzing" @click="resetQuickAnalysis">清空</el-button>
        </div>
      </el-form>

      <div v-if="analysisResult" class="analysis-result">
        <el-alert
          :title="analysisResult.has_fault ? '检测到故障迹象' : '未检测到故障迹象'"
          :type="analysisResult.has_fault ? 'error' : 'success'"
          :closable="false"
          show-icon
        />

        <el-descriptions :column="2" border size="small" class="analysis-result-grid">
          <el-descriptions-item label="文件名">{{ analysisResult.filename || quickAnalysis.filename || '手动输入' }}</el-descriptions-item>
          <el-descriptions-item label="结论">
            <el-tag :type="analysisResult.has_fault ? 'danger' : 'success'">
              {{ analysisResult.has_fault ? '故障日志' : '正常日志' }}
            </el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="置信度">{{ formatConfidence(analysisResult.confidence) }}</el-descriptions-item>
          <el-descriptions-item label="总行数">{{ analysisResult.line_count ?? '-' }}</el-descriptions-item>
          <el-descriptions-item label="ERROR">{{ analysisResult.log_level_stats?.error_count ?? 0 }}</el-descriptions-item>
          <el-descriptions-item label="WARNING">{{ analysisResult.log_level_stats?.warning_count ?? 0 }}</el-descriptions-item>
        </el-descriptions>

        <div class="analysis-summary">
          <div class="analysis-summary-label">摘要</div>
          <div>{{ analysisResult.summary || '-' }}</div>
        </div>

        <div v-if="analysisResult.extracted_error_codes?.length" class="analysis-summary">
          <div class="analysis-summary-label">错误码</div>
          <div class="analysis-tags">
            <el-tag
              v-for="code in analysisResult.extracted_error_codes"
              :key="code"
              type="danger"
              size="small"
            >
        ...
        <div v-if="analysisResult.extracted_stack_traces?.length" class="analysis-summary">
          <div class="analysis-summary-label">堆栈片段</div>
          <pre class="analysis-pre">{{ analysisResult.extracted_stack_traces[0] }}</pre>
        </div>
      </div>
    </el-card>
  </div>
...
const sectionOptions = [
  { label: '运行列表', value: 'runs' },
  { label: '快速分析', value: 'analysis' },
]
...
async function handleQuickAnalyze() {
  if (!quickAnalysis.logText.trim()) {
    ElMessage.warning('请输入日志内容')
    return
  }

  const requestId = ++quickAnalyzeRequestId
  analyzing.value = true
  analysisResult.value = null
  try {
    const result = await analyzeLog({
      filename: quickAnalysis.filename.trim() || 'manual_input.log',
      log_text: quickAnalysis.logText,
    })
    if (requestId !== quickAnalyzeRequestId) return
    analysisResult.value = result
  } catch (error) {
    if (requestId !== quickAnalyzeRequestId) return
    ElMessage.error(`日志分析失败: ${error.message}`)
  } finally {
    if (requestId === quickAnalyzeRequestId) {
...
function resetQuickAnalysis() {
  quickAnalysis.filename = ''
  quickAnalysis.logText = ''
  analysisResult.value = null
}

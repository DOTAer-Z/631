...
4. 支持统一窗口策略：fault run -> error window，normal run -> time window
"""
...
def fetch_mysql_labels(cur, case_id: Optional[str], task_id: Optional[str], is_fault: Optional[bool]) -> Dict:
    result = {
        "fault_type": None,
        "root_cause": None,
...
def resolve_strategy(requested_strategy: str, run_meta: Dict) -> str:
    if requested_strategy != "auto":
        return requested_strategy
    return "error" if bool(run_meta.get("is_fault")) else "time"
...
def main() -> None:
    ...
    ap.add_argument(
        "--strategy",
        choices=["time", "error", "auto"],
        default="time",
        help="time/error 为固定切窗；auto 使用统一策略：fault run -> error，normal run -> time",
    )
    ...
    try:
        for i, run_id in enumerate(run_ids, 1):
            ...
            effective_strategy = resolve_strategy(args.strategy, run_meta)
            print(f"[INFO] effective_strategy={effective_strategy}")

            if effective_strategy == "time":
            ...
            if not args.skip_mysql_summary:
                with mysql_conn.cursor() as cur:
                    ...
                    upsert_feature(cur, task_id, "window_strategy", effective_strategy, "string")
                mysql_conn.commit()

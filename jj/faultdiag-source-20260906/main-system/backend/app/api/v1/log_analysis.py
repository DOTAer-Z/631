...
from app.schemas.log_analysis import (
    ...
    AnalyzeApiRequest, AnalyzeLogResult,
...
# ── POST /log-analysis/analyze/api（接口导入，纯分析，不写 MySQL） ───────────
@router.post("/log-analysis/analyze/api", response_model=AnalyzeLogOut)
async def analyze_api(body: AnalyzeApiRequest):
    import httpx
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            if body.method.upper() == "GET":
                response = await client.get(body.url, params=body.params, headers=body.headers, timeout=30)
            else:  # POST
                response = await client.post(body.url, json=body.params, headers=body.headers, timeout=30)
            
            response.raise_for_status()  # 检查响应状态码
            text = response.text
    except httpx.HTTPError as e:
        raise HTTPException(status_code=500, detail="上传源连接失败，请检查地址、权限或网络状态")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"接口请求失败: {str(e)}")
    
    return _analyze_text(text, "接口导入")
...
@router.post("/log-analysis/upload/api")
async def upload_log_api(data: Dict[str, Any] = Body(...)):
    """
    从接口导入日志并存入 MongoDB。
    """
    url = data.get("url")
    ...
    try:
        ...
        return {"run_id": run_id, "filename": filename, "file_size": len(text), "status": "success"}
    except Exception as exc:
        logger.error(f"接口导入日志失败: {exc}")
        raise HTTPException(status_code=500, detail=f"日志存储失败: {str(exc)}")
...
@router.post("/log-analysis/analyze/log/api", response_model=AnalyzeLogOut)
async def analyze_log_api(data: AnalyzeApiRequest):
    """
    从接口导入并分析日志。
    """
    import httpx
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            if data.method.upper() == "GET":
                response = await client.get(data.url, params=data.params, headers=data.headers, timeout=30)
            else:  # POST
                response = await client.post(data.url, json=data.params, headers=data.headers, timeout=30)
    ...
    return _analyze_text(text, "接口导入", source_type="api", api_url=data.url)

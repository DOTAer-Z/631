# 故障定位 + 日志标注 集成系统 · Kubernetes 部署手册（631change 6 镜像版）
...
`631change`（2026-08）。在 631(7.6) 集成基线上：
...
```bash
# 构建上下文是 631change 的【上级目录】，依赖同级 model_train/
cd /home/junjiezuo/631-fault
docker build -f 631change/main-system/backend/Dockerfile.training -t faultdiag-training-worker:latest .
...
## 631change（2026-08：二进制过滤 + LLM 网关）

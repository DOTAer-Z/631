...
class TrainingMaterializerTests(unittest.TestCase):
    ...
    def _jsonl_version(
        ...
        *,
        ...
        sample_class: str = "fault",
        fault_type: str | None = "watchdog_timeout",
    ) -> int:
        ...
        digest = hashlib.sha256(
            "".join(content for _round, content in logs).encode("utf-8")
        ).hexdigest()
        ...
        for round_no, content in logs:
            self.db.add(TrainingTestLog(
    ...
    def test_structured_jsonl_materializes_cpt_without_legacy_auxiliary_logs(self):
        version_id = self._jsonl_version(
            platform="nuttx",
            test_name="Test_50005",
            logs=[(1, "fault round log"), (2, "normal round log")],
            fault_type=None,
        )
        task = self._task("cpt", {"train": version_id})

        materialized = materialize_task(self.db, task, self.root / "structured-cpt")
        rows = [json.loads(line) for line in materialized.train_file.read_text().splitlines()]

        self.assertEqual([row["round"] for row in rows], ["round_1", "round_2"])
        self.assertEqual({row["source_platform"] for row in rows}, {"nuttx"})
        self.assertIn("fault round log", rows[0]["text"])
        self.assertIn("normal round log", rows[1]["text"])

    def test_segment_jsonl_materializes_cpt_and_sft_from_one_text_stream(self):
        version_id = self._jsonl_version(
            platform="synthetic",
            test_name="seg_001",
            logs=[(1, "maintenance report: storage exhausted")],
            fault_type="storage_full",
        )

        cpt = materialize_task(
            self.db,
            self._task("cpt", {"train": version_id}),
            self.root / "segment-cpt",
        )
        cpt_rows = [json.loads(line) for line in cpt.train_file.read_text().splitlines()]
        self.assertEqual(len(cpt_rows), 1)
        self.assertEqual(cpt_rows[0]["round"], "round_1")

        self.db.query(TrainingTaskTest).delete()
        self.db.query(TrainingTask).delete()
        self.db.commit()
        sft = materialize_task(
            self.db,
            self._task("sft", {"train": version_id}),
            self.root / "segment-sft",
        )
        sft_rows = [json.loads(line) for line in sft.train_file.read_text().splitlines()]
        self.assertTrue(sft_rows)
        user_prompt = sft_rows[0]["messages"][1]["content"]
        self.assertEqual(user_prompt.count('<log round="'), 1)
        self.assertIn("maintenance report: storage exhausted", user_prompt)
        assistant = json.loads(sft_rows[0]["messages"][2]["content"])
        self.assertEqual(assistant["fault_type"], "storage_full")

    def test_structured_jsonl_sft_requires_fault_type_but_accepts_assigned_label(self):
        unlabeled_id = self._jsonl_version(
            platform="nuttx",
            test_name="Test_unlabeled",
            logs=[(1, "fault evidence"), (2, "normal baseline")],
            fault_type=None,
        )
        task = self._task("sft", {"train": unlabeled_id})
        with self.assertRaisesRegex(MaterializationError, "missing fault type labels"):
            materialize_task(self.db, task, self.root / "structured-sft-unlabeled")

        version = self.db.get(TrainingTestVersion, unlabeled_id)
        version.fault_type = "scheduler_deadlock"
        version.ground_truth = {
            **version.ground_truth,
            "fault_type": "scheduler_deadlock",
        }
        version.fip_info = {"FAULT_TYPE": "scheduler_deadlock"}
        self.db.commit()

        materialized = materialize_task(self.db, task, self.root / "structured-sft-labeled")
        rows = [json.loads(line) for line in materialized.train_file.read_text().splitlines()]
        self.assertTrue(rows)
        self.assertIn("scheduler_deadlock", rows[0]["messages"][2]["content"])

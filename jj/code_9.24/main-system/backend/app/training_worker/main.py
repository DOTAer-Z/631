...
def base_model_status(base_model_path: str | Path) -> str:
    ...
    if path.exists() and os.access(path, os.R_OK):
        return "ready"
    # 演示/流程展示：允许缺失基座模型时仍视为 ready，否则 worker 会因 model_status
    # != "ready" 一直 _defer_claim，队列里的任务永远停在「排队中」。
...
def _base_model_marker_ready(base_model_path: str | Path) -> bool:
    ...
    if marker.is_file() and os.access(marker, os.R_OK):
        return True
    # 演示/流程展示：同 base_model_status，允许缺失基座模型。
    return bool(getattr(settings, "TRAINING_ALLOW_MISSING_MODEL", False))


def _gpu_ready(environ: dict[str, str], gpu_probe: GpuSnapshotProvider) -> bool:
    # The compose stack exposes exactly one host GPU and remaps it to CUDA 0
    # inside the container.  The host index may be 0, 1, or another numeric
    # device; do not hard-code a particular host GPU here.
    visible = (environ.get("NVIDIA_VISIBLE_DEVICES") or "").strip()
    if visible != "all" and not visible.isdigit():
        return False
...
def _default_device_probe(
    ...
    if requested_device == "cpu":
        ...
        device_type = (
            "cuda"
            if (environ.get("NVIDIA_VISIBLE_DEVICES") or "").strip() == "all"
            or (environ.get("NVIDIA_VISIBLE_DEVICES") or "").strip().isdigit()

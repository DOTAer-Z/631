...
_JSONL_PLATFORMS = {"nuttx", "synthetic"}
    ...
    with _exclusive_task_lock(task_root):
        ...
        try:
            ...
            if task.task_type == "cpt":
                rows_by_split = {
                    split: _build_cpt_rows(samples_by_split[split], split, MAX_LOG_CHARS)
                    for split in SPLITS
                }
            else:
                _validate_sft_labels(samples_by_split)
...
def _sample_from_version(
    ...
    if platform in _JSONL_PLATFORMS:
        return _sample_from_jsonl_version(
            version,
            platform=platform,
            test_name=test_name,
            log_by_key=log_by_key,
        )
...
def _sample_from_jsonl_version(
    version: TrainingTestVersion,
    *,
    platform: str,
    test_name: str,
    ...
    """Build a sparse TestSample for structured/segment JSONL training data.

    The legacy NuttX directory path remains strict and never enters this helper.
    JSONL sources legitimately contain only qemu_console text, so absent event/
    metric streams are represented as empty strings and omitted by the flexible
    SFT renderer.
    """
    unsupported = set(log_by_key) - {
        (1, "qemu_console"),
        (2, "qemu_console"),
    }
    if unsupported:
        raise MaterializationError(
            f"Frozen JSONL Test version {version.id} has unsupported logs: {sorted(unsupported)}"
        )
    if not any((text or "").strip() for text in log_by_key.values()):
        raise MaterializationError(
            f"Frozen JSONL Test version {version.id} has no usable log text"
        )
    if platform == "synthetic" and (2, "qemu_console") in log_by_key:
        raise MaterializationError(
            f"Frozen segment Test version {version.id} has an unexpected round_2 log"
        )
    ...
    return TestSample(
        ...
        ground_truth=dict(version.ground_truth),
        fip_info=dict(version.fip_info),
        round1_qemu=log_by_key.get((1, "qemu_console"), ""),
        round2_qemu=log_by_key.get((2, "qemu_console"), ""),
        round1_events="",
        round2_events="",
        round1_metrics="",
        round2_metrics="",
...
def _build_cpt_rows(
    ...
    """Keep legacy samples byte-for-byte compatible; omit absent JSONL rounds."""
    legacy = [sample for sample in samples if sample.path.parent.name not in _JSONL_PLATFORMS]
    flexible = [sample for sample in samples if sample.path.parent.name in _JSONL_PLATFORMS]
    rows = build_cpt_rows(legacy, split_name, max_chars)
    for sample in flexible:
        available_rounds = {
            round_name
            for round_name, text in (
                ("round_1", sample.round1_qemu),
                ("round_2", sample.round2_qemu),
            )
            if (text or "").strip()
        }
        rows.extend(
            row
            for row in build_cpt_rows([sample], split_name, max_chars)
            if row.get("round") in available_rounds
        )
    # Keep compatibility with tests/callers that monkeypatch build_cpt_rows with
    # minimal rows lacking the optional id field.
    return sorted(rows, key=lambda row: str(row.get("id", "")))


def _validate_sft_labels(samples_by_split: dict[str, list[TestSample]]) -> None:
    """Fault samples need a target label; normal samples do not."""
    missing = sorted(
        sample.test_id
        for samples in samples_by_split.values()
        for sample in samples
        if sample.path.parent.name in _JSONL_PLATFORMS
        and sample.ground_truth.get("sample_class") != "normal"
        and not str(sample.ground_truth.get("fault_type") or "").strip()
    )
    if missing:
        preview = ", ".join(missing[:5])
        suffix = "..." if len(missing) > 5 else ""
        raise MaterializationError(
            f"JSONL SFT samples are missing fault type labels: {preview}{suffix}"
        )

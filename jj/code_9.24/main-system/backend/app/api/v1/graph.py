...
@router.get("/graph/options")
def get_graph_options(
    node_type: str = Query(..., description="FaultType | RootCause | Subsystem | Case"),
):
    """Return all KG nodes usable as focus values in the graph selector."""
    return {"items": GraphService().list_options(node_type)}


@router.get("/graph")
def get_graph(
    db: Session = Depends(get_db),
    view: str = Query("overview", description="overview | fault_type | root_cause | subsystem | case"),

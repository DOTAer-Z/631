...
def _task_out_values(
    ...
    # A terminal successful task is complete even when the last persisted
    # training metric reports an intermediate epoch (for example 0.47).
    # Keep the task lifecycle state authoritative for the public progress.
    if task.state == "succeeded":
        progress = 1.0
    elif latest_metric is not None and latest_metric.epoch is not None:
        progress = latest_metric.epoch
    else:

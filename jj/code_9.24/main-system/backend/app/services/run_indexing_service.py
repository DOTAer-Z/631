...
# 只从日志中提取有明确语法或明确设备前缀的实体，避免把普通英文单词
# 猜成组件。抽取结果始终带有窗口原文证据，便于审计和后续规则扩展。
_COMPONENT_PATTERNS = (
    re.compile(
        r"\b(?:component|module|service|driver|device|interface|adapter|controller)"
        r"\s*[:=]\s*([A-Za-z][A-Za-z0-9_.:/-]{1,80})",
        re.IGNORECASE,
    ),
)
_KNOWN_COMPONENT_PATTERN = re.compile(
    r"\b(?:eth\d+|ens\d+|eno\d+|enp\d+s\d+(?:f\d+)?|wlan\d+|wwan\d+|"
    r"mmc\d+|nvme\d+(?:n\d+)?|sd[a-z]\d*|ttyS?\d+|"
    r"i2c[-_]?\d+|spi[-_]?\d+|can\d+|usb\d+|gpu\d+|cpu\d+|qemu|systemd)"
    r"(?:[-_.:/][A-Za-z0-9_-]+)*\b",
    re.IGNORECASE,
)
_EVENT_RULES = (
    ("out_of_memory", re.compile(r"\bout[ -]?of[ -]?memory\b|\boom\b|内存不足", re.IGNORECASE)),
    ("permission_denied", re.compile(r"permission denied|access denied|权限拒绝", re.IGNORECASE)),
    ("verification_failed", re.compile(r"checksum|校验失败|验证失败|integrity check", re.IGNORECASE)),
    ("initialization_failed", re.compile(r"(?:failed to )?initiali[sz]e|initialization failed|初始化失败", re.IGNORECASE)),
    ("timeout", re.compile(r"\btime[ -]?out\b|timed out|超时", re.IGNORECASE)),
    ("link_down", re.compile(r"link down|link is down|disconnected|断连|链路.*断", re.IGNORECASE)),
    ("resource_not_found", re.compile(r"no such (?:device|file|directory)|not found|不存在|找不到", re.IGNORECASE)),
    ("packet_loss", re.compile(r"packet loss|packet drop|丢包", re.IGNORECASE)),
    ("process_crash", re.compile(r"\b(?:crash|crashed|panic|segmentation fault)\b|崩溃|宕机", re.IGNORECASE)),
    ("operation_failed", re.compile(r"\b(?:failed|failure|error|exception)\b|失败|异常", re.IGNORECASE)),
)


def _canonical_graph_name(value: Any) -> str:
    return re.sub(r"\s+", " ", str(value or "").strip()).strip()


def _extract_graph_facts(text: str) -> Dict[str, List[Dict[str, str]]]:
    """Deterministically extract auditable component/event facts from one window."""
    source = (text or "")[:_MAX_TEXT_CHARS]
    components: Dict[str, Dict[str, str]] = {}
    for pattern in _COMPONENT_PATTERNS:
        for match in pattern.finditer(source):
            value = _canonical_graph_name(match.group(1)).strip(".,;()[]{}")
            if len(value) >= 2:
                key = value.casefold()
                components.setdefault(key, {"name": value, "evidence": match.group(0)[:160]})
    for match in _KNOWN_COMPONENT_PATTERN.finditer(source):
        value = _canonical_graph_name(match.group(0)).strip(".,;()[]{}")
        if len(value) >= 2:
            key = value.casefold()
            components.setdefault(key, {"name": value, "evidence": match.group(0)[:160]})

    events: Dict[str, Dict[str, str]] = {}
    for pattern_name, pattern in _EVENT_RULES:
        match = pattern.search(source)
        if match:
            events.setdefault(
                pattern_name,
                {"name": pattern_name, "evidence": match.group(0)[:160]},
            )
    return {
        "components": list(components.values()),
        "event_patterns": list(events.values()),
    }


def _stable_window_id(run_id: str, window_id: Any) -> str:
    raw = f"{run_id}\x00{window_id}"
...
def index_runs(
    ...
    *,
    ...
    for run_id in run_ids:
        ...
        system_name = ""
        if getattr(run, "system_id", None):
            system = db.query(System).filter(System.system_id == run.system_id).first()
            system_name = (system.name if system is not None else str(run.system_id)).strip()
        graph_windows_by_doc_id: Dict[str, Dict[str, Any]] = {}
        ...
        for u in units:
            ...
            facts = _extract_graph_facts(body)
            graph_windows_by_doc_id[doc_id] = {
                "window_id": _stable_window_id(run_id, u.get("key")),
                "source_id": str(u.get("key") or ""),
                "source_kind": u.get("kind") or "entry",
                "text_sha256": hashlib.sha256(body.encode("utf-8", errors="replace")).hexdigest(),
                "start_time": str(u.get("start_time")) if u.get("start_time") is not None else None,
                "end_time": str(u.get("end_time")) if u.get("end_time") is not None else None,
                "components": facts["components"],
                "event_patterns": facts["event_patterns"],
            }
            meta: Dict[str, Any] = {
        ...
        if not force:
            ...
            existing_doc_ids = {
                doc_ids[i] for i, metadata in enumerate(existing) if metadata is not None
            }
        ...
        successful_doc_ids = _upsert_batched(vs, doc_ids, texts, metadatas, errors, run_id)
        run_indexed = len(successful_doc_ids)
        run_failed = len(doc_ids) - run_indexed
        kg_doc_ids = existing_doc_ids | successful_doc_ids
        graph_windows = [
            graph_windows_by_doc_id[doc_id]
            for doc_id in graph_windows_by_doc_id
        ...
        if not skip_kg and graph_windows:
            kg_instances.append({
                "run_id": run_id,
                "case_id": run.case_id,
...
def _upsert_batched(
    ...
    """分批 upsert，返回成功 doc_id；失败逐批记录不中断整体。"""
    ok: set[str] = set()

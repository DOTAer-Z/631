...
class GraphService:
    ...
    def build_graph(
        ...
        if view == "fault_type":
            ...
            result = self._build_case_graph(kg_index, focus)
        else:
    ...
    def list_options(self, node_type: str) -> list[dict]:
        """Return all stable KG nodes for the focus selector."""
        allowed = {"FaultType", "RootCause", "Subsystem", "Case"}
        ...
        if kg_index is None:
            return []
        items = []
        for node in kg_index["nodes_by_type"].get(node_type, []):
            ...
            items.append({"id": node.get("id"), "label": str(label), "type": node_type})
        return sorted(items, key=lambda item: (item["label"].casefold(), item["id"]))
    ...
    def _build_overview_graph(self, kg_index: dict) -> dict:
        ...
        for ft_id in ft_ids:
            for e in in_edges.get(ft_id, []):
                if raw_nodes.get(e["source"], {}).get("type") == "RootCause":
                    rc_score[e["source"]] += 1
        top_rc = sorted(rc_score, key=rc_score.__getitem__, reverse=True)[:12]
        ...
        top_struct = sorted(struct_score, key=struct_score.__getitem__, reverse=True)[:12]
        selected.update(top_struct)

        # Step 4：每个入选 FaultType 挂多个 Case 实例，形成充实的总览子图。
        # 「数据导入 → 批量入知识库」写入的是 Case 节点，总览图若不展示它们，
        # 用户会以为数据没进图谱（原实现只挑 FaultType/RootCause/结构节点）。
        case_seen: Set[str] = set()
        for ft_id in ft_ids:
            added_for_ft = 0
            for e in in_edges.get(ft_id, []):
                cid = e["source"]
                if raw_nodes.get(cid, {}).get("type") == "Case" and cid not in case_seen:
                    case_seen.add(cid)
                    selected.add(cid)
                    added_for_ft += 1
                    if added_for_ft >= 3 or len(case_seen) >= 24:
                        break

        # Step 5：展示部分实例窗口，以及窗口命中的组件/事件模式。
        # 底层可保存全部窗口，总览只抽样，避免图过密。
        window_seen: Set[str] = set()
        detail_seen: Set[str] = set()
        for case_id in list(case_seen)[:12]:
            for e in out_edges.get(case_id, []):
                target = e["target"]
                target_type = raw_nodes.get(target, {}).get("type")
                if target_type in ("System", "Subsystem", "RootCause"):
                    selected.add(target)
                if target_type != "LogWindow" or len(window_seen) >= 16:
                    continue
                window_seen.add(target)
                selected.add(target)
                for we in out_edges.get(target, []):
                    did = we["target"]
                    if (
                        raw_nodes.get(did, {}).get("type") in ("Component", "EventPattern")
                        and did not in detail_seen
                    ):
                        detail_seen.add(did)
                        selected.add(did)
                        if len(detail_seen) >= 16:
                            break
    ...
    def _build_fault_type_graph(self, kg_index: dict, focus: Optional[str]) -> dict:
        ...
        外围：RootCause / Case / System / Subsystem / Component / LogWindow / EventPattern
        """
        raw_nodes    = kg_index["raw_nodes"]
        out_edges    = kg_index["out_edges"]
        in_edges     = kg_index["in_edges"]
        ...
        # Case 是故障类型的实例层，从案例继续展开系统、子系统、窗口和事件。
        case_ids = [
            e["source"] for e in in_edges.get(focus_id, [])
            if raw_nodes.get(e["source"], {}).get("type") == "Case"
        ][:16]
        priority.extend(case_ids)
        detail_seen: Set[str] = set()
        for case_id in case_ids:
            for e in out_edges.get(case_id, []):
                target = e["target"]
                target_type = raw_nodes.get(target, {}).get("type")
                if target_type in ("System", "Subsystem"):
                    priority.append(target)
                elif target_type == "LogWindow":
                    priority.append(target)
                    for we in out_edges.get(target, []):
                        did = we["target"]
                        if raw_nodes.get(did, {}).get("type") in ("Component", "EventPattern"):
                            detail_seen.add(did)
                            if len(detail_seen) >= 12:
                                break
            if len(detail_seen) >= 12:
                break
        priority.extend(detail_seen)
        ...
        selected = set(priority[:60])
        result = self._assemble_graph(selected, kg_index)
        logger.info("[GraphService] fault_type view focus=%s: %d nodes", focus_id, len(result["nodes"]))
        return result

    def _build_case_graph(self, kg_index: dict, focus: Optional[str]) -> dict:
        """案例焦点图（9.16 新增），目标节点数 ≤ 15。

        中心：1 个 Case 节点（focus 指定，或取连接度最高的）；
              focus 也可传 run_id（`Case::<run_id>` 会命中）。
        外围：该 Case 的 FaultType(≤1) + RootCause(≤4) + Subsystem(≤2)，
              以及同故障类型下的兄弟 Case(≤6) —— 便于横向对比同类样本。
        """
        raw_nodes     = kg_index["raw_nodes"]
        out_edges     = kg_index["out_edges"]
        in_edges      = kg_index["in_edges"]
        ...
        focus_id = _find_node_id(focus, "Case", raw_nodes)
        if not focus_id and focus:
            # 允许直接传 run_id
            cand = f"Case::{focus}"
            if cand in raw_nodes:
                focus_id = cand
        if not focus_id:
        ...
        # Case → FaultType / RootCause / Subsystem / LogWindow
        ft_ids: List[str] = []
        window_ids: List[str] = []
        for e in out_edges.get(focus_id, []):
            t = raw_nodes.get(e["target"], {}).get("type")
            if t == "FaultType" and len(ft_ids) < 1:
                ft_ids.append(e["target"])
            elif t == "RootCause" and len(priority) < 6:
                priority.append(e["target"])
            elif t == "Subsystem" and len(priority) < 8:
                priority.append(e["target"])
            elif t == "LogWindow" and len(window_ids) < 4:
                window_ids.append(e["target"])
        priority.extend(ft_ids)
        priority.extend(window_ids)

        # 每个窗口最多展示一个组件和一个事件模式。
        detail_seen: Set[str] = set()
        for wid in window_ids:
            per_type: Set[str] = set()
            for e in out_edges.get(wid, []):
                target = e["target"]
                node_type = raw_nodes.get(target, {}).get("type")
                if node_type not in ("Component", "EventPattern") or node_type in per_type:
                    continue
                per_type.add(node_type)
        ...
        for ft_id in ft_ids:
            for e in in_edges.get(ft_id, []):
                sid = e["source"]
                if (
                    raw_nodes.get(sid, {}).get("type") == "Case"
                    and sid != focus_id
                    and sid not in sib_seen
                ):
                    sib_seen.add(sid)
                    priority.append(sid)
                    if len(sib_seen) >= 6:
        ...
        selected = set(priority[:60])
        result = self._assemble_graph(selected, kg_index)
        logger.info("[GraphService] case view focus=%s: %d nodes", focus_id, len(result["nodes"]))
        return result
    ...
    def _build_root_cause_graph(self, kg_index: dict, focus: Optional[str]) -> dict:
        ...
        根因焦点图，目标节点数 ≤ 15。
        中心：1 个 RootCause
        外围：FaultType / Case / System / Subsystem / Component / LogWindow / EventPattern
        """
        raw_nodes    = kg_index["raw_nodes"]
        out_edges    = kg_index["out_edges"]
        in_edges     = kg_index["in_edges"]
        ...
        for case_id in case_ids:
            for e in out_edges.get(case_id, []):
                target = e["target"]
                target_type = raw_nodes.get(target, {}).get("type")
                if target_type in ("System", "Subsystem", "LogWindow"):
                    priority.append(target)
                if target_type == "LogWindow":
                    for we in out_edges.get(target, []):
                        did = we["target"]
                        if raw_nodes.get(did, {}).get("type") in ("Component", "EventPattern"):
                            detail_seen.add(did)
    ...
    def _build_subsystem_graph(self, kg_index: dict, focus: Optional[str]) -> dict:
        ...
        子系统焦点图，目标节点数 ≤ 15。
        中心：1 个 Subsystem
        外围：Case / FaultType / RootCause / System / LogWindow / Component / EventPattern
        """
        raw_nodes    = kg_index["raw_nodes"]
        out_edges    = kg_index["out_edges"]
        in_edges     = kg_index["in_edges"]
        ...
        # 当前导入数据中 Case -> OCCURRED_IN -> Subsystem 是最可靠的关系，
        # 因此先展开案例，再展开案例的故障、根因、窗口和窗口细节。
        case_ids = [
            e["source"] for e in in_edges.get(focus_id, [])
            if raw_nodes.get(e["source"], {}).get("type") == "Case"
        ][:24]
        ...
        for case_id in case_ids:
            for e in out_edges.get(case_id, []):
                target = e["target"]
                target_type = raw_nodes.get(target, {}).get("type")
                if target_type in ("System", "FaultType", "RootCause"):
                    priority.append(target)
                elif target_type == "LogWindow":
                    priority.append(target)
                    for we in out_edges.get(target, []):

...
class VectorStoreService:
    ...
    def upsert_documents(
        ...
        """批量 upsert：同 doc_id 已存在则覆盖（幂等重入知识库的基础）。

        与 add_documents 的区别：add 遇到重复 doc_id 会抛
        `DuplicateIDError`，导致「同一条 run 二次入库」直接失败。
        批量入知识库必须可重复执行，故用 upsert。
        """
        if not doc_ids:
            return
        self.collection.upsert(ids=doc_ids, documents=texts, metadatas=metadatas)

    def get_metadatas(self, doc_ids: List[str]) -> List[Optional[dict]]:
        """按 doc_id 批量回查 metadata，用于判断「哪些已入库」。

        不存在的 id 在 Chroma 返回里缺失，故按传入顺序补齐 None，
        保证 返回值长度 == len(doc_ids)、与入参一一对应。
        """
        if not doc_ids:
            return []
        try:
            res = self.collection.get(ids=doc_ids, include=["metadatas"])
        except Exception as exc:  # noqa: BLE001
            logger.warning(f"Chroma get 失败，视为全部未入库: {exc}")
            return [None] * len(doc_ids)
        found = {
            _id: (meta or {})
            for _id, meta in zip(res.get("ids") or [], res.get("metadatas") or [])
        }
        return [found.get(_id) for _id in doc_ids]
    ...
    def count_by_prefix(self, prefix: str) -> int:
        """统计 doc_id 以 prefix 开头的向量数。

        Chroma 的 doc_id 不是 metadata 字段，不能 where 过滤，只能全量取 id
        再前缀匹配。仅在「按 run 看入库进度」这种小规模统计场景调用。
        """
        try:
            res = self.collection.get(include=[])
            return sum(1 for _id in (res.get("ids") or []) if str(_id).startswith(prefix))
        except Exception as exc:  # noqa: BLE001
            logger.warning(f"Chroma 统计前缀 {prefix} 失败: {exc}")
            return 0

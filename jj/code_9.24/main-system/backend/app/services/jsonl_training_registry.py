...
def register_training_for_jsonl(
    ...
    *,
    ...
    if version is None:
        ...
        # 已有 complete 版本：追加另一个 round，同时合并标签。
        # 结构化 Test 的 round_1 是故障轮、round_2 是正常基线轮；只要出现
        # round_1，整个 Test 样本就按 fault 训练，不能被后导入的 round_2
        # 覆盖成 normal。
        merged_class = version.sample_class
        if platform == "nuttx":
            if round_no == 1 or sample_class == "fault":
                merged_class = "fault"
            elif not merged_class:
                merged_class = sample_class
        else:
            merged_class = sample_class or merged_class

        merged_fault_type = fault_type or version.fault_type
        ground_truth = dict(version.ground_truth or {})
        ground_truth.update({
            "sample_class": merged_class,
            "domain": domain or version.domain,
            "fault_type": merged_fault_type,
            "label": merged_class,
        })
        version.ground_truth = ground_truth
        version.sample_class = merged_class
        version.domain = domain or version.domain
        version.fault_type = merged_fault_type
        if merged_fault_type:
            fip_info = dict(version.fip_info or {})
            fip_info["FAULT_TYPE"] = merged_fault_type

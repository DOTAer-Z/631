...
def build_run_filters(
    *,
    ...
    # 数据源类型：按 run_id 前缀归类（与 infer_data_category 保持一致）
    if data_category:
        if data_category == "segment":
            clauses.append("r.run_id LIKE :dc_seg")
            params["dc_seg"] = "seg_%"
        elif data_category == "semi":
            clauses.append("r.run_id LIKE :dc_semi")
            params["dc_semi"] = "nuttx_%"
        elif data_category == "structured":
            clauses.append("r.run_id LIKE :dc_struct")
            params["dc_struct"] = "Test_%"
...
def infer_data_category(row: Dict[str, Any]) -> str:
    """根据 run 的 system_id + run_id 前缀推导「数据源类型」。

    - structured   : 结构化日志（Test_xxx:round_x:qemu_console 来自 test.jsonl / struct.jsonl）
    - segment      : 非结构化语义段（seg_xxx，来自 notstruct/segments.jsonl）
    - semi         : 半结构化训练集（nuttx_Test_xxx_round_x，NuttX Test_* 目录）
    - unknown      : 其它/旧数据
    """
    system_id = row.get("system_id")
    run_id = row.get("run_id") or ""
    if system_id == "synthetic" or run_id.startswith("seg_"):
        return "segment"
    if run_id.startswith("nuttx_") or (system_id == "nuttx" and not run_id.startswith("Test_")):
        return "semi"
    if run_id.startswith("Test_") or system_id == "nuttx":
        return "structured"
    return "unknown"
...
def shape_run_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "system_id": row.get("system_id"),
        "data_category": infer_data_category(row),
...
def _sync_jsonl_training_labels(db, *, run, case) -> int:
    """Propagate file-browser labels to structured/segment training versions.

    Legacy semi-structured NuttX versions are deliberately excluded: their
    immutable ground_truth.json/fip_info.data snapshot remains authoritative.
    """
    from app.models.run import Run
    from app.models.training_data import TrainingTest, TrainingTestVersion

    if run.system_id not in {"nuttx", "synthetic"}:
        return 0

    related_runs = [run]
    if case is not None:
        related_runs = db.query(Run).filter(Run.case_id == case.case_id).all() or [run]

    fault_type = (case.fault_type_label or "").strip() if case is not None else ""
    sample_class = "fault" if bool(case.is_fault if case is not None else run.is_fault) else "normal"
    synced: set[int] = set()

    for related in related_runs:
        if related.system_id not in {"nuttx", "synthetic"}:
            continue
        version = None
        if related.test_version_id:
            version = db.get(TrainingTestVersion, related.test_version_id)
        if version is None:
            test_name = related.run_id if related.system_id == "synthetic" else related.test_name
            if not test_name:
                continue
            training_test = (
                db.query(TrainingTest)
                .filter_by(platform=related.system_id, test_name=test_name)
                .one_or_none()
            )
            if training_test is not None and training_test.latest_version_id:
                version = db.get(TrainingTestVersion, training_test.latest_version_id)
                related.test_version_id = training_test.latest_version_id
        if version is None or version.id in synced:
            continue
        version_test = db.get(TrainingTest, version.test_id)
        if version_test is None or version_test.platform not in {"nuttx", "synthetic"}:
            continue

        ground_truth = dict(version.ground_truth or {})
        ground_truth["sample_class"] = sample_class
        ground_truth["label"] = sample_class
        ground_truth["fault_type"] = fault_type or None
        version.ground_truth = ground_truth
        version.sample_class = sample_class
        version.fault_type = fault_type or None

        fip_info = dict(version.fip_info or {})
        if fault_type:
            fip_info["FAULT_TYPE"] = fault_type
        else:
            fip_info.pop("FAULT_TYPE", None)
        version.fip_info = fip_info
        synced.add(version.id)
...
def update_run(db, *, run_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if case is not None and ({"fault_type_id", "is_fault"} & set(payload)):
        if _sync_jsonl_training_labels(db, run=run, case=case):
            updated.append("training_label")
...
def delete_run(
    ...
    *,
    ...
    # 5. 知识图谱：删掉该 run 的 Case 节点及其边（9.16：run 可能已入 KG）
    deleted_kg_nodes = 0
    deleted_kg_edges = 0
    try:
        ...
        kg_res = kg_service_v2.remove_run_cases([run_id])
        deleted_kg_nodes = kg_res.get("removed_nodes", 0)
        deleted_kg_edges = kg_res.get("removed_edges", 0)
        if deleted_kg_nodes or deleted_kg_edges:
    ...
    return {
        ...
        "deleted_kg_nodes": deleted_kg_nodes,
        "deleted_kg_edges": deleted_kg_edges,
    }

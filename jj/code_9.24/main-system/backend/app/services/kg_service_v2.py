...
class KGServiceV2:
    ...
    def append_run_cases(
        ...
        *,
        ...
        生成的节点：
            ...
            System::<name>        （有系统字段才建）
            LogWindow::<stable-id>（每个实际入库窗口/回退日志条目）
            Component::<name>     （日志中命中明确组件语法时）
            EventPattern::<name>  （日志中命中确定性事件规则时）
        生成的边（去重）：
            Case     -HAS_RESULT_FAULT->  FaultType      （即 kg 文档里的实例关系）
            Case     -HAS_ROOT_CAUSE   -> RootCause
            Case     -OCCURRED_IN      -> Subsystem
            Case     -BELONGS_TO      -> System
            Case     -HAS_LOG_WINDOW  -> LogWindow
            LogWindow- MENTIONS_COMPONENT -> Component
            LogWindow- MATCHES_EVENT   -> EventPattern
            LogWindow- INDICATES      -> FaultType
            RootCause- CAUSES_FAULT    -> FaultType
            FaultType- CAUSED_BY       -> RootCause
        ...
        with self._lock:
            ...
            for inst in instances:
                ...
                system = (inst.get("system") or "").strip()
                if system:
                    system_id = f"System::{system.casefold()}"
                    _ensure_node(system_id, "System", {"name": system})
                    _ensure_edge(case_id, "BELONGS_TO", system_id, run_id)

                # 窗口级实例层：只接受上游规则抽取出的稳定 ID 和证据，
                # 不由 KG 层猜测文本，保证可追溯、可重复导入。
                for window in inst.get("windows") or []:
                    window_id = (window.get("window_id") or "").strip()
                    if not window_id:
                        continue
                    log_id = f"LogWindow::{window_id}"
                    window_props = {
                        "run_id": run_id,
                        "source_id": window.get("source_id"),
                        "source_kind": window.get("source_kind") or "window",
                        "text_sha256": window.get("text_sha256"),
                        "start_time": window.get("start_time"),
                        "end_time": window.get("end_time"),
                    }
                    _ensure_node(log_id, "LogWindow", window_props)
                    _ensure_edge(case_id, "HAS_LOG_WINDOW", log_id, run_id)

                    for component in window.get("components") or []:
                        name = (component.get("name") or "").strip()
                        if not name:
                            continue
                        component_id = f"Component::{name.casefold()}"
                        _ensure_node(
                            component_id,
                            "Component",
                            {"name": name, "normalization": "deterministic-rule"},
                        )
                        _ensure_edge(
                            log_id,
                            "MENTIONS_COMPONENT",
                            component_id,
                            component.get("evidence") or run_id,
                        )

                    for event in window.get("event_patterns") or []:
                        name = (event.get("name") or "").strip()
                        if not name:
                            continue
                        event_id = f"EventPattern::{name.casefold()}"
                        _ensure_node(
                            event_id,
                            "EventPattern",
                            {"name": name, "normalization": "deterministic-rule"},
    ...
    def remove_run_cases(self, run_ids: List[str]) -> dict:
        ...
        Case 和窗口实例会被删除；故障类型、根因、子系统以及仍被其它窗口
        引用的组件/事件模式保留。规则抽取且已成为孤儿的组件/事件模式会清理。
        返回 {removed_nodes, removed_edges, rewritten}。
        """
        case_targets = {f"Case::{r}" for r in run_ids if r}
        if not case_targets:
        ...
        with self._lock:
            ...
            # Case 和其窗口是实例节点；组件/事件模式只有在规则抽取且成为孤儿时清理。
            window_targets = {
                node_id
                for node_id, node in self._nodes.items()
                if node.get("type") == "LogWindow"
                and node.get("properties", {}).get("run_id") in set(run_ids)
            }
            targets = case_targets | window_targets
            ...
            # 组件和事件模式由日志窗口规则抽取；当已无任何窗口引用时，
            # 删除这些孤儿抽象节点，避免类别计数与实际知识不一致。
            referenced = {
                endpoint
                for edges in self._out_edges.values()
                for edge in edges
            ...
            orphan_targets = {
                ...
                for node_id, node in self._nodes.items()
                if (
                    node.get("type") in {"Component", "EventPattern"}
                    and node.get("properties", {}).get("normalization") == "deterministic-rule"
                )
                and node_id not in referenced
            }
            for node_id in orphan_targets:
                self._nodes.pop(node_id, None)
                self._out_edges.pop(node_id, None)
                self._in_edges.pop(node_id, None)
            targets |= orphan_targets
            removed_nodes += len(orphan_targets)

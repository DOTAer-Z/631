...
test('buildRunListParams adds data_category only when provided', () => {
  assert.deepEqual(
    buildRunListParams({ page: 1, pageSize: 20, dataCategory: 'segment' }),
    { page: 1, page_size: 20, data_category: 'segment' },
  )
  // 默认不强制 system_id → 后端返回全部，三类（结构化/非结构化/半结构化）都可见
  assert.deepEqual(
    buildRunListParams({ page: 1, pageSize: 20 }),
    { page: 1, page_size: 20 },
  )
})

test('buildRunListParams passes any of the three data categories', () => {
  for (const cat of ['structured', 'segment', 'semi']) {
    assert.deepEqual(
      buildRunListParams({ page: 1, pageSize: 20, dataCategory: cat }),
      { page: 1, page_size: 20, data_category: cat },
    )
  }
})

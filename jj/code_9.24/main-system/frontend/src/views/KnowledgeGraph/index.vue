...
import { getGraph, getGraphOptions } from '@/api/graph'
...
const optionTypeByView = {
  fault_type: 'FaultType',
  root_cause: 'RootCause',
  subsystem: 'Subsystem',
  case: 'Case',
}
const focusPlaceholder = computed(() => ({
  fault_type: '选择故障类型',
  root_cause: '选择根因',
  subsystem: '选择子系统',
  case: '选择案例',
...
async function handleViewChange() {
  ...
  if (viewMode.value !== 'overview') {
    ...
    try {
      const result = await getGraphOptions(optionTypeByView[viewMode.value])
      focusOptions.value = result.items || []
    } catch (error) {
      loadError.value = `加载焦点列表失败：${error.message || error}`
    } finally {

        ...
        <el-table :data="tasks" size="small" row-key="id" v-loading="taskLoading">
          ...
          <el-table-column label="进度" width="150">
            <template #default="{ row }">
              <el-progress v-if="row.progress != null" :percentage="progressPercent(row.progress, row.state)" :stroke-width="8" />
...
function progressPercent(value, state) {
  if (state === 'succeeded') return 100
  return Math.max(0, Math.min(100, Math.round(Number(value) <= 1 ? Number(value) * 100 : Number(value))))
}

            ...
            <el-form-item>
              <el-space>
                ...
                <el-button
                  ...
                  :loading="pushLoading"
                  :disabled="currentPendingIndex < 0"
                  data-testid="workbench-push-button"
                  @click="pushToMainWindow"
                >
                  推送到主系统分析
                </el-button>
...
import { getSliceWindowFull, pushSliceWindow } from '@annotation/api/sliceWindows'
...
async function pushToMainWindow() {
  const ctx = selectedWindowCtx.value
  if (!ctx) {
    ElMessage.warning('当前没有可推送窗口')
    return
  }
  pushLoading.value = true
  try {
    const result = await pushSliceWindow(ctx.window_id)
    ElMessage.success(`已推送到主系统：${result.run_id}`)
  } catch (e) {
    const code = isAxiosError(e) ? (e.response?.data as { error_code?: string } | undefined)?.error_code : undefined
    if (isAxiosError(e) && e.response?.status === 502) {
      ElMessage.error('推送失败：主系统校验未通过，请检查窗口是否有有效日志与标注')
    } else if (code === 'WINDOW_NO_TIMESTAMP') {
      ElMessage.error('推送失败：该窗口内没有带有效时间戳的日志行，主系统无法对其分析')
    } else {

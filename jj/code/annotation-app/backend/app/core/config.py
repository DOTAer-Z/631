...
class Settings(BaseSettings):
    ...
    # 标注保存后自动推送到主系统「文件选择」（集成方案 A 的反向桥）。
    # True：每次成功保存/更新窗口标注后，自动把该窗口推送到主系统
    #       POST /log-analysis/annotated-run，落成一条 run（run_id=annot_pkg{pid}_win{wid}），
    #       使其出现在主系统文件选择并可按标注判定分流到故障诊断/预测预警。
    #       —— 这就是「标注成功后自动进入文件选择的数据列表」的一环。
    # False：仅保留工作台的「推送到主系统分析」手动按钮。
    annotation_auto_push_to_main: bool = Field(
        default=True,
        validation_alias=AliasChoices(
            "ANNOTATION_AUTO_PUSH_MAIN",
            "annotation_auto_push_to_main",
        ),
    )
    # 自动推送专用超时（秒）：主系统短暂不可达时，避免用默认 60s 拖慢标注保存。
    # 自动推送失败不阻断标注保存（标注仍是成功的），只会让该窗口暂时缺席文件选择，
    # 事后可经「推送到主系统分析」按钮补推。
    annotation_auto_push_timeout_seconds: int = Field(
        default=10,
        ge=1,
        le=120,
        validation_alias=AliasChoices(
            "ANNOTATION_AUTO_PUSH_TIMEOUT_SECONDS",
            "annotation_auto_push_timeout_seconds",
        ),
    )
    ...
    def llm_enabled(self) -> bool:
        # 零-yaml：LLM 能力只取决于网关地址是否配置（网关在集群内直连，
        # 已去掉共享 token）。token 字段保留仅为兼容旧 env，不再参与判定。

...
def prediction_history(page: int = 1, page_size: int = 20, db: Session = Depends(get_db)):
    # 预测预警（判定为「正常」的文件）与故障诊断（判定为「异常」的文件）按 verdict 互斥：
    # 已被判定为故障(has_fault=True)的 run 从预测历史里剔除，避免同一 run 两个入口都出现。
    from app.database import get_mongo_db
    try:
        fault_run_ids = {
            d.get("run_id")
            for d in get_mongo_db()["log_analysis_results"].find(
                {"has_fault": True, "run_id": {"$ne": None}}, {"run_id": 1}
            )
        }
    except Exception:  # 集合不存在或不可用时不阻断历史查询
        fault_run_ids = set()

    q = db.query(PredictionRecord).order_by(PredictionRecord.created_at.desc())
    if fault_run_ids:
        q = q.filter(PredictionRecord.run_id.notin_(fault_run_ids))
    total = q.count()

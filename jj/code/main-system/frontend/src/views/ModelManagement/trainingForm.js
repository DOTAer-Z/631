...
export function buildTaskPayload(form) {
  ...
  if (form.base_model_path && form.base_model_path.trim()) {
    payload.base_model_path = form.base_model_path.trim()
  }
  return payload
}
...
// ── 数据源类型：平台值 → 结构化 / 非结构化 / 半结构化 ──
// 训练文件选择里 platform 字段原始值大小写/命名不一致（需按大小写区分）：
//   nuttx（小写） → 结构化（Test_xxx:round_x:qemu_console，round_1=故障、round_2=正常）
//   NuttX（大写） → 半结构化（NuttX/Test_xxx 目录，含 fip_info.data / ground_truth.json）
//   synthetic     → 非结构化（seg_xxx 语义段，自带 ground_truth）
function _categoryFor(platform) {
  const raw = String(platform || '')
  if (raw === 'synthetic') return 'segment'
  if (raw === 'NuttX') return 'semi'
  if (raw === 'nuttx') return 'structured'
  return 'semi'
}

export function platformCategory(platform) {
  return _categoryFor(platform)
}

export function platformCategoryLabel(platform) {
  const map = { structured: '结构化', segment: '非结构化', semi: '半结构化' }
  return map[_categoryFor(platform)] || '其他'
}

export function platformCategoryType(platform) {
  const map = { structured: 'success', segment: 'warning', semi: 'info' }
  return map[_categoryFor(platform)] || 'info'
}

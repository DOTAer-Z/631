  ...
  <div class="data-import-page">
    ...
    <el-card shadow="never" class="list-card">
      ...
      <el-table :data="rows" size="small" v-loading="loading">
        ...
        <el-table-column label="数据源类型" width="150">
          <template #default="{ row }">
            <el-tag :type="formatOrigin(row.format) === '带标签数据' ? 'success' : 'info'" size="small">
        ...
        <el-table-column label="摄入统计" width="280">
          <template #default="{ row }">
            <div v-if="row.format && row.ingest_status === 'ingesting'" class="ingest-stats">
              <el-progress
                :percentage="jsonlProgressOf(row)"
                :stroke-width="10"
                :format="() => `${row.ingest_status === 'cancelled' ? '取消中' : (jsonlTasks[row.import_id]?.processed_records || 0)} / ${jsonlTasks[row.import_id]?.total_records || '?'}`"
              />
            </div>
        ...
        <el-table-column label="操作" width="320" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              v-if="row.format && row.ingest_status === 'ingesting'"
              link
              type="danger"
              @click="handleCancelJsonl(row)"
            >
              取消
            </el-button>
...
function formatLabel(format) {
  if (!format) return 'NuttX 训练集'
  const parts = String(format).split(',').map((s) => s.trim()).filter(Boolean)
  return parts
    .map((p) => (p === 'structured' ? '结构化 JSONL' : p === 'segment' ? '非结构化段 JSONL' : p))
    .join(' / ')
}

function formatOrigin(format) {
  if (!format) return '未标注数据'
  return String(format).split(',').map((s) => s.trim()).filter(Boolean).some((p) => p === 'structured' || p === 'segment')
    ? '带标签数据'
    : 'NuttX 训练集'
}
...
// jsonl 导入任务进度：import_id → { state, processed, total, ... }
const jsonlTasks = reactive({})

async function pollJsonlTasks() {
  for (const row of rows.value) {
    if (row.format && (row.ingest_status === 'ingesting' || row.ingest_status === 'pending')) {
      try {
        const t = await getJsonlTask(row.import_id)
        if (t && t.task_id) jsonlTasks[row.import_id] = t
      } catch {
        /* 无任务或未识别为 jsonl，忽略 */
      }
    }
...
function jsonlProgressOf(row) {
  const t = jsonlTasks[row.import_id]
  if (!t || !t.total_records) return 0
  return Math.min(100, Math.round((t.processed_records / t.total_records) * 100))
}

async function handleCancelJsonl(row) {
  try {
    await cancelJsonlTask(row.import_id)
    ElMessage.success('已请求取消 jsonl 导入')
    await loadList()
  } catch (e) {

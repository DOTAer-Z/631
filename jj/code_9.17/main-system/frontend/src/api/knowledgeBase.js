...
// ── 9.16 新增：把「文件选择」里的 run 批量入知识库(Chroma) + 知识图谱(KG) ──
// 单位是 log_window（诊断检索的粒度）；未标注故障类型的 run 会被后端 skip。
export const indexRuns = (data) => request.post('/knowledge-base/index-runs', data)
// 查一批 run 的入库状态（列表「已入知识库」列）
export const getIndexStatus = (data) => request.post('/knowledge-base/index-status', data)
// 把 run 从知识库/知识图谱中移除（改故障类型后重算用）
export const removeRunIndex = (data) => request.post('/knowledge-base/remove-run-index', data)

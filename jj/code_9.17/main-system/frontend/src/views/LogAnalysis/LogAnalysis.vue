    ...
    >
      ...
      >
        <template #default>
          <div style="font-size: 12px; line-height: 1.7;">
            ...
            · <strong>未标注故障类型</strong>的 run 会被跳过 —— 请先在「编辑」里选好故障类型再入库。<br />
      ...
      <div class="index-preview">
        <el-tag
          ...
          :key="row.run_id"
          size="small"
          :type="row.fault_type ? 'success' : 'info'"
          effect="plain"
          class="meta-tag"
        >{{ row.fault_type || '未标注故障类型' }} · {{ row.run_id }}</el-tag>
        <span v-if="selectedRows.length > 30" class="muted">
          …等共 {{ selectedRows.length }} 条
        </span>
      </div>
      <p v-if="unlabeledCount > 0" class="unlabeled-warn">
        其中 {{ unlabeledCount }} 条未标注故障类型，将被跳过。
      </p>
...
const unlabeledCount = computed(
  () => selectedRows.value.filter((r) => !r.fault_type).length,
)
...
async function submitIndexRuns() {
  ...
  const labeled = selectedRows.value.filter((r) => r.fault_type).length
  if (labeled === 0) {
    ElMessage.warning('选中的 run 都没有标注故障类型，入知识库会被全部跳过。请先「编辑」选故障类型。')
    return
  }
  if (unlabeledCount.value > 0) {
    try {
      await ElMessageBox.confirm(
        `${unlabeledCount.value} 条 run 未标注故障类型，将被跳过。是否继续？`,
        '确认入库',
  ...
  try {
    ...
    if (problems.length) {
      ...
      ElMessage.info(`有 ${problems.length} 条 run 被跳过，原因见浏览器控制台`)
    }

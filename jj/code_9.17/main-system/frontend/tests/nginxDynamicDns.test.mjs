...
test('root nginx template uses fixed upstreams without resolver and without DNS injection', async () => {
  ...
  // 真·深融合：标注 SPA 已编进主 bundle，旧的 /annotate/ 独立站反代块被**刻意移除**。
  // 这里锁定它不得回归——若重新出现，说明有人把标注退回 iframe 模式。
  assert.doesNotMatch(config, /location \/annotate\/ \{/)
  assert.doesNotMatch(config, /proxy_pass http:\/\/frontend-annotate:80;/)
...
test('Docker image installs the root config as a template and runs the custom entrypoint', async () => {
  ...
  // 真·深融合：build context 已扩散到仓库根（vite @annotation 别名指向 main-system/frontend
  // 之外），故 Dockerfile 内所有 COPY 源路径都带 `main-system/frontend/` 前缀。
  assert.match(
    dockerfile,
    /COPY main-system\/frontend\/nginx\/default\.conf \/etc\/nginx\/templates\/default\.conf\.template/,
  )
  assert.match(dockerfile, /COPY main-system\/frontend\/docker-entrypoint\.sh \/docker-entrypoint\.sh/)
  assert.match(dockerfile, /RUN chmod \+x \/docker-entrypoint\.sh/)
  assert.match(dockerfile, /ENTRYPOINT \["\/docker-entrypoint\.sh"\]/)
  assert.doesNotMatch(dockerfile, /COPY \S*nginx\/default\.conf \/etc\/nginx\/conf\.d\/default\.conf/)
})